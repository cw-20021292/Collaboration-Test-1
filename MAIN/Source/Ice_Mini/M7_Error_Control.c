...
#define         Bit0_Cold_Operation_Disable_State                 U8OperationB.Bit.b0       // 냉각 정지 에러
#define         Bit1_Hot_Operation_Disable_State                  U8OperationB.Bit.b1       // 온수 정지 에러
#define         Bit2_Ice_Operation_Disable_State                  U8OperationB.Bit.b2       // 제빙 정지 에러
...
void error_management(void)
{
    ...
    || (Bit0_Hot_Water_Flow_Block_Error__E08 == SET && u8WaterOutState == HOT_WATER_SELECT))
    {
        ...
        /* 에러발생 시 동시 추출 관련된 데이터 클리어 250731 CH.PARK */
    ...
    if(   Bit2_Room_Temp_Open_Short_Error__E42 == SET
       ...
       /*..sean [25-01-20] 직수식이어서 pass..*/
       /*|| Bit4_Room_Low_Water_Level_Error__E21 == SET*/

       /*..hui [23-9-20오전 9:24:23] 둘 다 고장..*/
       /* 외기온도 센서 둘다 고장인 경우 에러는 판단하지만 제빙은 디폴트값으로 동작한다 250630 CH.PARK */
